...
function Contact() {
  return (
    <footer id="contact" className="py-[5rem] px-28 flex flex-col gap-12">
      <div className="flex gap-12">
        <h2 className="text-5xl font-normal  tracking-[-0.02em] w-full max-w-[31rem]">
          Get in Touch
        </h2>
        <p className="font-light text-xl text-muted w-full max-w-[29rem]">
          ...
          {listOfSocials.map((link) => (
            <li key={link.id} className="flex gap-2 items-center">
              ...
              <a
                href={link.link}
                target={link.target}
                className="text-primary font-geist  text-2xl  opacity-80 hover:opacity-100 transition-opacity"
